...
def main(cfg: Config | None = None) -> None:
    ...
    print("   -> Computing decadal persistence (run lengths) and F8 maps...")
    aquifer_persist = compute_aquifer_persistence(aquifer_dec_df, cfg)
    if aquifer_persist is not None and not aquifer_persist.empty:
        ...
        multi_persist = compute_aquifer_persistence_for_periods(aquifer_dec_df, cfg)
        for label, d_start, d_end in MULTI_TREND_PERIODS:
